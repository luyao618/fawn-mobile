...
const exactAndroidDiagnosticCommands = [
...
function assertAndroidDiagnosticsPolicy(script: string, workflow: Workflow): void {
  ...
  assert.deepEqual(
    diagnosticCommands,
    exactAndroidDiagnosticCommands,
    "The executable Android diagnostic adb command inventory must exactly match in order",
  );
  const cleanupLines = [
    "cleanup() {",
    "  status=$?",
...
test("headless Metro, readiness, and iOS confirmation policies reject hostile counterexamples", async () => {
  ...
  for (const mutatedScript of androidPolicyMutations) {
    ...
    assert.throws(
      ...
      /Android cleanup must preserve|distinct exact debug-output directories|executable Android diagnostic adb command inventory must exactly match/,
    );
  }
